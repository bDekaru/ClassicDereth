...
void sha512_free(sha512_context *ctx)
{
	...
	zeroize(ctx, sizeof(sha512_context));
}
...
std::string SHA512(const void *input, size_t ilen)
{
	...
	SHA512((const unsigned char *)input, ilen, hash);

	char text[129];
	DataToHexString(hash, 64, text);
